...
export default function SignupPage() {
  ...
  const handleCheck = async () => {
    ...
    if (nationalId.length !== 14 || !/^\d{14}$/.test(nationalId)) {
      setCheckErr('National ID must be exactly 14 digits.'); return
    }
    setChecking(true)
    try {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      await api.post('/auth/signup', {
        username: form.username.trim(), password: form.password, nationalId,
      ...
      <form onSubmit={handleSubmit} className="card" style={{ width: 420 }}>
        ...
        <div style={{ display: 'flex', gap: 8 }}>
          <input value={nationalId}
            ...
            placeholder="12345678901234" maxLength={14} style={{ flex: 1 }}
